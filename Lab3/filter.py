...
def translate_text():
    try:
        ...
        if not check_file_exists(text_filename):
            print(f"Файл '{text_filename}' не знайдений.")
            return
    except Exception as e:
        return str(e)
    ...
    print(f"Размір файла: {os.path.getsize(text_filename)} байт")
    ...
    if num_characters > max_characters or num_words > max_words or num_sentences > max_sentences:
        print("Досягнутий ліміт символів, слів або речень.")
        return
    ...
    if output_destination == "file":
        ...
        with open(output_filename, "w", encoding="utf-8") as output_file:
            output_file.write(f"Мову переводу: {target_language}\n")
            output_file.write(translation)
        print("Ok")
    elif output_destination == "screen":
        print(f"Мова переводу: {target_language}")
        print(translation)
        print("Ok")
    else:
        print("Невідоме місце виводу результату.")
